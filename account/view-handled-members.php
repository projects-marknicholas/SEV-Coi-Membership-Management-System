...
<body>
	...
	<div class="flex-sidebar">
		...
		<main>
			<?php
				$my_sql = "SELECT * FROM coordinator WHERE coordinators_code = '".$_GET['viewid']."' ORDER BY id DESC";
				$my_res = mysqli_query($link, $my_sql);

				if (mysqli_num_rows($my_res) > 0) {
					while ($my_result = mysqli_fetch_assoc($my_res)) {
			?>
			<p class="page-title">You are viewing <span class="view-data">"<?php echo $my_result['coordinator']?><span style="text-transform: lowercase;">'s</span>"</span> handled members.</p>
			<?php
					}
				}
			?>
			...
			<div class="table-div">
				...
				<table id="table">
					...
					if (isset($_GET['viewid'])) {
						$sql = "SELECT * FROM member WHERE coordinators_code = '".$_GET['viewid']."' ORDER BY id DESC";
